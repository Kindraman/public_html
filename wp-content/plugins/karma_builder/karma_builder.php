...
if(!function_exists('truethemes_image_frame_constructor')):
	...
	return truethemes_formatter($output);
...
public function render_karma_builder_icon_box($atts, $content = null) {
	...
	*/
	...
	//Build output for icon
	if(!empty($type)){
		$icon_output = '[raw]<span class="fa-stack '.$icon_size.'"><i class="fa fa-circle fa-stack-2x"></i><i class="fa '.esc_attr( ${"icon_" . $type} ).' fa-stack-1x fa-inverse"></i></span>[/raw]';
	}
	
	//backward compatible, do not remove.
	if(!empty($icon)){
		$icon_output = '[raw]<span class="fa-stack '.$icon_size.'"><i class="fa fa-circle fa-stack-2x"></i><i class="fa '.esc_attr( $icon ).' fa-stack-1x fa-inverse"></i></span>[/raw]';
	}		
    ...
    //custom icon upload will overwrite the above custom icon and icon.
        $uploaded_custom_icon_image = wp_get_attachment_image($custom_icon_upload,'full');
        $icon_output = '[raw]<span class="karma-custom-icon-img">'.$uploaded_custom_icon_image.'</span>[/raw]';
    }	


	//build the shortcode
	if(!empty($lightbox_content)){
	$output = '[raw]<a href="'.$lightbox_content.'" data-gal="prettyPhoto" title="'.$lightbox_description.'" class="true-icon-box tt_'.$animate.' '.$custom_css_class.' '.$unique_id.'">[/raw]';
	} elseif(!empty($a_href)){
	$output = '[raw]<a href="'.$a_href.'" target="'.$a_target.'" title="'.$a_title.'" class="true-icon-box tt_'.$animate.' '.$custom_css_class.' '.$unique_id.'">[/raw]';
	} else {
	$output = '[raw]<div class="true-icon-box tt_'.$animate.' '.$custom_css_class.' '.$unique_id.'">[/raw]';
	}

	$output .= $icon_output.$content;
...
public function render_karma_builder_number_counter($atts, $content = null) {
	...
	return '<div class="true-counter-wrap tt_'.$animate.' '.$custom_css_class.' '.$unique_id.'"><h3 class="true-counter true-zero">'.$number.'</h3><h4>'.$title.'</h4></div>';
}// END shortcode
...
public function render_karma_builder_services( $atts, $content = null ) {      
    ...
    // Dynamic CSS Function
	$style_code ='.true-services.'.$unique_id.' .true-custom-icon,
	.true-services.'.$unique_id.' .true-karma-custom-icon{color:'.$icon_color.';border: '.$border_width.' solid '.$border_color.';background:'.$bg_color.';}
	.true-services.'.$unique_id.' .true-custom-icon:hover,
	.true-services.'.$unique_id.' .true-karma-custom-icon:hover{color:'.$icon_color_hover.';background-color:'.$bg_color_hover.';}';
...
$content =  '[raw]<div class="message_karma_'.$style.' colored_box tt_'.$animate.'" style="font-size:'.$font_size.';">[/raw]' . do_shortcode($content) . '[raw]</div><br class="clear" />[/raw]';
return truethemes_formatter($content);
}
...
public function render_karma_builder_one_sixth( $atts, $content = null ) {
   $content = '[raw]<div class="one_sixth tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div>[/raw]';
   return truethemes_formatter($content);
}
public function render_karma_builder_one_sixth_last( $atts, $content = null ) {
   $content = '[raw]<div class="one_sixth_last tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_one_fifth( $atts, $content = null ) {
   $content = '[raw]<div class="one_fifth tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div>[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_one_fifth_last( $atts, $content = null ) {
   $content = '[raw]<div class="one_fifth_last tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_one_fourth( $atts, $content = null ) {
   $content = '[raw]<div class="one_fourth tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div>[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_one_fourth_last( $atts, $content = null ) {
   $content = '[raw]<div class="one_fourth_last tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_one_third( $atts, $content = null ) {
   $content = '[raw]<div class="one_third tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div>[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_one_third_last( $atts, $content = null ) {
   $content = '[raw]<div class="one_third_last tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_one_half( $atts, $content = null ) {
   $content = '[raw]<div class="one_half tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div>[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_one_half_last( $atts, $content = null ) {
   $content = '[raw]<div class="one_half_last tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_two_thirds( $atts, $content = null ) {
   $content = '[raw]<div class="two_thirds tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div>[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_two_thirds_last( $atts, $content = null ) {
   $content = '[raw]<div class="two_thirds_last tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_three_fourth( $atts, $content = null ) {
   $content = '[raw]<div class="three_fourth tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div>[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_three_fourth_last( $atts, $content = null ) {
   $content = '[raw]<div class="three_fourth_last tt-column">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_builder_flash_wrap( $atts, $content = null ) {
   $content = '[raw]<div class="flash_wrap">[/raw]' . truethemes_formatter(do_shortcode($content)) . '[raw]</div><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
...
public function render_karma_builder_hr_shadow($atts, $content = null) {
    $content = '[raw]<div class="hr_shadow {$custom_css_class}">&nbsp;</div>[/raw]';  
   return truethemes_formatter($content);
}
//do not remove or comment out, leave for backward compatibility
public function render_karma_builder_hr($atts, $content = null) {
    $content = "[raw]<div class='hr {$custom_css_class}'>&nbsp;</div>[/raw]"; 
    return truethemes_formatter($content); 
}
public function render_karma_separator_line($atts, $content = null){
  ...
  if($style == 'separator_shadow'){
    $output = '[raw]<div class="hr_shadow {$custom_css_class}">&nbsp;</div>[/raw]';  
  }
  
  if($style == 'separator_clean_line'){
    $output = '[raw]<div class="hr {$custom_css_class}">&nbsp;</div>[/raw]';  
  }
...
public function render_karma_builder_sc_gap($atts, $content = null) {
  ...
  'size'            => '100px',
    ...
    $content = '[raw]<div class="hr_gap" style="height:'.$size.';"></div>[/raw]';
    return truethemes_formatter($content);
}
...
public function render_accordion( $atts, $content = null ){
    ...
    $output = '';
    $output .= '[raw]<ul class="accordion '.$class.' '.$custom_css_class.' tt_'.$animate.'">[/raw]';
...
public function render_tabset($atts, $content = null) {
    ...
    */
    $indiviual_tab_labels = explode(',',$tab_labels);
	    ...
	    $output = '';
	    $output .= '[raw]<div class="tabs-area tt_'.$animate.'">[/raw]';
...
public function render_testimonial_wrap( $atts, $content = null ) {
   ...
   $code = '[raw]<div class="true-testimonial-wrapper tt_'.$animate.' '.$custom_css_class.'"><div class="testimonials flexslider"><ul class="slides">' . do_shortcode($content) . '</ul></div></div><!-- END testimonials -->[/raw]';
   return truethemes_formatter($code);
}
...
public function render_team_member($atts, $content = null) {
  ...
  'last_item'      => '',
	...
	$output ='[raw]<div class="member-wrap tt_'.$animate.'[/raw]';
...
public function render_callout1( $atts, $content = null ) {
  $content ='[raw]<div class="callout-wrap"><span>' . do_shortcode($content) . '</span></div><!-- END callout-wrap --><br class="clear" />
[/raw]';  
 return truethemes_formatter($content);
...
public function render_callout2( $atts, $content = null ) {
   $content = '[raw]<p class="callout2"><span>' . do_shortcode($content) . '</span></p><br class="clear" />[/raw]';
    return truethemes_formatter($content);
}
public function render_karma_callout_text($atts, $content = null){
  ...
  $output = '[raw]<div class="callout-wrap tt_'.$animate.' '.$custom_css_class.'"><span>' . do_shortcode($content) . '</span></div><!-- END callout-wrap --><br class="clear" />
[/raw]';  
  }

  if($style=='karma_callout_2'){
   $output = '[raw]<p class="callout2 '.$custom_css_class.'"><span>' . do_shortcode($content) . '</span></p><br class="clear" />[/raw]';
  }
...
public function render_video_left( $atts, $content = null ) {
   $output = '[raw]<div class="video-wrap video_left">[/raw]' . do_shortcode($content) . '[raw]</div><!-- END video-wrap -->[/raw]';
     return truethemes_formatter($output);
}
public function render_video_right( $atts, $content = null ) {
   $output = '[raw]<div class="video-wrap video_right">[/raw]' . do_shortcode($content) . '[raw]</div><!-- END video-wrap -->[/raw]';
     return truethemes_formatter($output);
}
...
public function render_video_text( $atts, $content = null ) {
   $output = '[raw]<div class="video-sub">[/raw]' . do_shortcode($content) . '[raw]</div><!-- END video-sub --><br class="clear" />[/raw]';
     return truethemes_formatter($output);
}
...
//old blog_posts shortcode layouts. 
		...
		if($layout == 'default'){
		$output = '[raw]<div class="blog-posts-shortcode-outer-wrap tt_'.$animate.'"><ul class="tt-recent-posts">[/raw]';
		}else{
		$output = '[raw]<div class="blog-posts-shortcode-outer-wrap tt_'.$animate.'">[/raw]';
		}
		...
		//$output .= '[raw]<span class="tt-blog-placeholder tt-blog-default tt-blog-sc-img fa fa-file-text-o">&nbsp;</span>[/raw]';
		endif; 
...
public function render_karma_list( $atts, $content = null ) {
   ...
   $list = '[raw]<ul class="list tt_'.$animate.' '.$custom_css_class.' fa-ul">' . $content . '</ul>[/raw]';
   return truethemes_formatter($list);
}
...
public function render_karma_list_item( $atts, $content = null ){
	...
	return truethemes_formatter($list_html);
}
...
public function karma_builder_enqueue_script() {
	//JavaScript
	wp_enqueue_script( 'karma-builder-bootstrap-js', plugins_url('js/bootstrap.min.js', __FILE__), '1.0', array(), $infooter = true );
	wp_enqueue_script( 'appear', plugins_url('js/appear.min.js', __FILE__), '3.2', array('jquery'), $infooter = true );
	wp_enqueue_script( 'waypoints', plugins_url('js/waypoints.min.js', __FILE__), '4.0', array('jquery'), $infooter = true );
	wp_enqueue_script( 'easyCharts', plugins_url('js/easy-pie-chart.min.js', __FILE__), '1.0.1', array('jquery'), $infooter = true );
	wp_enqueue_script( 'karma-builder', plugins_url('js/karma-builder.js', __FILE__), '1.0', array('jquery'), $infooter = true );

	//CSS
	wp_enqueue_style( 'karma-builder', plugins_url('css/karma-builder.css', __FILE__) );
	//wp_enqueue_style( 'karma-builder-bootstrap', plugins_url('css/karma-builder-bootstrap.css', __FILE__) ); @since 1.2 CSS included at bottom of karma_builder.css
}
...
function karma_builder_extend_vc_class(){
	...
	if ( class_exists( 'WPBakeryShortCodesContainer' ) ) {
        ...
        // Grow Boxes class
	    class WPBakeryShortCode_karma_builder_grow_boxes extends WPBakeryShortCodesContainer {
	    }  
